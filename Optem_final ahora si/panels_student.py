...
class PanelAgendaSemanal(ctk.CTkFrame):
    ...
    def _exportar_pdf(self):
        """Recopila actividades visibles y las exporta a PDF/HTML."""
        ...
        for a in todos:
            ...
            cat       = a.get("categoria") or a.get("category") or ""
            desc      = a.get("descripcion") or a.get("description") or ""
            try:
                from datetime import date as _date
                partes = str(fecha_str).split("-")
                if len(partes) == 3:
                    fd = _date(int(partes[0]), int(partes[1]), int(partes[2]))
                    if lunes <= fd <= fin:
                        actividades.append({
                            "titulo": titulo, "fecha": fecha_str,
                            "hora": hora, "categoria": cat, "descripcion": desc,
                        })
            except Exception:
                pass
