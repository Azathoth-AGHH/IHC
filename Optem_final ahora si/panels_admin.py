...
class PanelAlumnos(ctk.CTkFrame):
    ...
    def _render_batch(self, items, start, batch=8):
        ...
        for a in items[start:end]:
            ...
            carga_fr = ctk.CTkFrame(info, fg_color="transparent")
            ...
            ctk.CTkLabel(carga_fr, text=f"📋 Carga: {tareas} tareas activas",
                font=("Helvetica", FS("small")), text_color=C("text2")).pack(side="left", padx=(0,8))
            ...
            if riesgo > 0:
                ctk.CTkLabel(carga_fr, text=f"  ⚠️ {riesgo} mat. en riesgo",
                    font=("Helvetica", FS("small"), "bold"), text_color=C("red")).pack(side="left")
            # ── Stats derechos ────────────────────────────────────
            ...
            Btn(right, text="Ver detalle", width=100, height=32,
                command=lambda n=nom, c=correo, s=sem, pr=prom, r=racha, t=tareas, ri=riesgo, num=nc:
...
class PanelHorarios(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        busq = ctk.CTkEntry(left, textvariable=self._busq_var,
                            placeholder_text="🔍 Buscar maestro...",
                            height=34, corner_radius=8,
                            fg_color=C("surface2"), text_color=C("text"),
                            border_color=C("border"))
        busq.pack(fill="x", padx=10, pady=(0, 6))
        self._busq_var.trace_add("write", lambda *_: self._filtrar_maestros())
