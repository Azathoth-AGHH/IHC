...
class PanelInicioEst(ctk.CTkScrollableFrame):
    ...
    def _build(self):
        ...
        # ── Botón Pomodoro rápido ─────────────────────────────────
        pom_card = Card(self)
        pom_card.pack(fill="x", padx=20, pady=(0,16))
        pom_inner = ctk.CTkFrame(pom_card, fg_color="transparent")
        pom_inner.pack(fill="x", padx=18, pady=16)
        ctk.CTkLabel(pom_inner, text="⏱  Sesión Pomodoro rápida",
                     font=("Helvetica", FS("h3"), "bold"), text_color=C("text")
                     ).pack(side="left")
        ctk.CTkButton(pom_inner, text="▶  Iniciar ahora", width=160, height=42,
                      corner_radius=12, fg_color=C("accent"),
                      hover_color=_darken(C("accent"), .15), text_color="white",
                      font=("Helvetica", FS("body"), "bold"),
                      command=lambda: self._ir_pomodoro_rapido()
                      ).pack(side="right")
    ...
    def _ir_pomodoro_rapido(self):
        """Navega a Pomodoro e inicia el timer automáticamente."""
        top = self.winfo_toplevel()
        app = getattr(top, "_app_screen", None)
        if app and hasattr(app, "ir_pomodoro_autostart"):
            app.ir_pomodoro_autostart()
        else:
            self.on_tab("Pomodoro")
...
class PanelActividades(ctk.CTkFrame):
    ...
    def _act_card(self, a):
        ...
        if es_tarea:
            ...
            ctk.CTkLabel(info,
                ...
                font=("Helvetica",FS("small"),"bold"),
                text_color=C("red"), cursor="hand2").pack(anchor="w", pady=(4,0))
        elif a.get("desc"):
            ctk.CTkLabel(info, text=a["desc"][:90]+("…" if len(a["desc"])>90 else ""),
                font=("Helvetica",FS("small")), text_color=C("text3"),
                cursor="hand2").pack(anchor="w")

        # Hint de que es clickable
        ctk.CTkLabel(inner, text="›", font=("Helvetica",18,"bold"),
            text_color=C("text3"), cursor="hand2").pack(side="right", padx=(0,4))

        # Toda la tarjeta abre el diálogo de info + IA
        _fn = lambda e, _a=a: self._ver_actividad_ia(_a)
        card.bind("<Button-1>", _fn)
        inner.bind("<Button-1>", _fn)
        for _w in inner.winfo_children():
            _w.bind("<Button-1>", _fn)
            for _ww in _w.winfo_children():
                _ww.bind("<Button-1>", _fn)
    ...
    def _ver_actividad_ia(self, a):
        """Diálogo unificado: información completa + IA interactiva."""
        top = ctk.CTkToplevel(self)
        top.title(a.get("titulo", "Actividad"))
        top.geometry("560x580")
        top.configure(fg_color=C("bg"))
        top.grab_set(); top.lift(); top.focus_force()

        cat  = a.get("categoria","")
        col_map = {"Estudio":C("accent"),"Tarea":C("accent"),"Deporte":C("green"),
                   "Ocio":C("teal"),"Reunión":C("amber"),"Salud":C("red"),"Personal":C("pink")}
        color = next((v for k,v in col_map.items() if k in cat), C("text3"))
        es_tarea = any(k in cat for k in ("Tarea","Examen","Práctica"))
        es_clase = "Estudio" in cat or "Clase" in a.get("titulo","")

        # Banda de color superior
        ctk.CTkFrame(top, fg_color=color, height=6, corner_radius=0).pack(fill="x")

        hdr = ctk.CTkFrame(top, fg_color="transparent")
        hdr.pack(fill="x", padx=24, pady=(16,4))
        ctk.CTkLabel(hdr, text=a.get("titulo","Sin título"),
            font=("Helvetica",FS("h2"),"bold"), text_color=C("text"),
            wraplength=400).pack(side="left", anchor="w")

        # Botones de editar/eliminar (esquina)
        action_fr = ctk.CTkFrame(hdr, fg_color="transparent")
        action_fr.pack(side="right")
        puede_editar = not (es_tarea or es_clase)
        if puede_editar:
            ctk.CTkButton(action_fr, text="✏️", width=32, height=32, corner_radius=8,
                fg_color=C("surface2"), text_color=C("text"), hover_color=C("accent_bg"),
                command=lambda: [top.destroy(), self._editar(a)]).pack(side="left", padx=2)
            ctk.CTkButton(action_fr, text="🗑", width=32, height=32, corner_radius=8,
                fg_color=C("surface2"), text_color=C("red"), hover_color="#FFE0E0",
                command=lambda: [top.destroy(), self._eliminar(a)]).pack(side="left", padx=2)

        ctk.CTkLabel(top,
            text=f"📅  {a.get('fecha','—')}    ·    {a.get('hora_inicio','?')} – {a.get('hora_fin','?')}    ·    {cat}",
            font=("Helvetica",FS("small")), text_color=C("text2")).pack(padx=24, anchor="w")

        if es_tarea:
            ctk.CTkLabel(top,
                text=f"🕐  Cierre: {a.get('fecha','—')} a las {a.get('hora_fin','23:59')}",
                font=("Helvetica",FS("small"),"bold"), text_color=C("red")).pack(padx=24, pady=(4,0), anchor="w")

        ctk.CTkFrame(top, fg_color=C("border"), height=1).pack(fill="x", padx=24, pady=10)

        # Área de texto para desc / respuesta IA
        txt_body = ctk.CTkTextbox(top, height=160, corner_radius=10,
            font=("Helvetica",FS("body")),
            fg_color=C("surface"), text_color=C("text"),
            border_color=C("border"), border_width=1)
        txt_body.pack(fill="x", padx=24)
        desc_init = a.get("desc") or "Sin descripción disponible."
        txt_body.insert("0.0", desc_init)
        txt_body.configure(state="disabled")

        ctk.CTkFrame(top, fg_color=C("border"), height=1).pack(fill="x", padx=24, pady=10)

        # Sección IA
        ctk.CTkLabel(top, text="🤖  Asistente IA — ¿En qué puedo ayudarte?",
            font=("Helvetica",FS("body"),"bold"), text_color=C("text")).pack(padx=24, anchor="w")
        lbl_st = ctk.CTkLabel(top, text="Elige una opción para que la IA te ayude.",
            font=("Helvetica",FS("small")), text_color=C("text2"))
        lbl_st.pack(padx=24, pady=(4,8), anchor="w")

        def _ia_accion(modo):
            prompts = {
                "resolver":  f"Soy estudiante. Tengo una actividad: '{a.get('titulo','')}'. Descripción: {a.get('desc','sin descripción')}. ¿Puedes ayudarme a resolverla o darme pasos concretos?",
                "resumen":   f"Haz un resumen breve y claro de esta actividad académica: '{a.get('titulo','')}'. Descripción: {a.get('desc','sin descripción')}.",
                "extra":     f"Dame información adicional, contexto o recursos útiles sobre el tema: '{a.get('titulo','')}'. Descripción: {a.get('desc','sin descripción')}.",
            }
            lbl_st.configure(text="⏳  Consultando IA...", text_color=C("accent"))
            top.update()
            try:
                from academic_engine import AcademicEngine
                engine = AcademicEngine({"agenda":[], "materias":{}})
                resultado = engine.generar_resumen_tarea(prompts[modo])
                txt_body.configure(state="normal")
                txt_body.delete("0.0","end")
                txt_body.insert("0.0", resultado)
                txt_body.configure(state="disabled")
                lbl_st.configure(text="✅  Respuesta lista.", text_color=C("green"))
            except Exception as ex:
                lbl_st.configure(text=f"⚠  {ex}", text_color=C("red"))

        ia_row = ctk.CTkFrame(top, fg_color="transparent")
        ia_row.pack(padx=24, pady=(0,10), anchor="w")
        Btn(ia_row, text="🆘  Ayudarme a resolver", width=190,
            command=lambda: _ia_accion("resolver")).pack(side="left", padx=(0,6))
        Btn(ia_row, text="📋  Resumen", width=120,
            command=lambda: _ia_accion("resumen")).pack(side="left", padx=(0,6))
        Btn(ia_row, text="📚  Info extra", width=120,
            command=lambda: _ia_accion("extra")).pack(side="left")

        BtnOutline(top, text="Cerrar", width=100,
...
class PanelAgendaSemanal(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        # ── Botón Exportar PDF ────────────────────────────────────
        ctk.CTkButton(ih, text="📥 Exportar PDF", width=130, height=32,
            corner_radius=10,
            font=("Helvetica", FS("small"), "bold"),
            fg_color=C("accent"), hover_color=C("accent_dark"), text_color="white",
            command=self._exportar_pdf).pack(side="right", padx=(0, 4))
    ...
    def _exportar_pdf(self):
        """Recopila actividades de la vista actual y las exporta a PDF."""
        from datetime import timedelta
        hoy   = datetime.now().date()
        todos = cargar_personal(self.file_key) + cargar_global()
        ...
        if self._vista == "dia":
            ...
            actividades = []
            for a in todos:
                fecha_str = a.get("fecha") or a.get("date") or ""
                if str(fecha_str) == dia_str:
                    actividades.append({
                        "titulo":      a.get("titulo") or a.get("title") or a.get("nombre") or "Sin título",
                        "fecha":       fecha_str,
                        "hora":        a.get("hora") or a.get("time") or "",
                        "categoria":   a.get("categoria") or a.get("category") or "",
                        "descripcion": a.get("descripcion") or a.get("description") or "",
                    })
            titulo_pdf = f"Agenda · {dia_obj.strftime('%d/%m/%Y')}"
            ...
            fin    = datetime(y, m, _cal.monthrange(y, m)[1]).date()
            actividades = []
            for a in todos:
                fecha_str = a.get("fecha") or a.get("date") or ""
                try:
                    ...
                    if len(partes) == 3:
                        fd = date(int(partes[0]), int(partes[1]), int(partes[2]))
                        if inicio <= fd <= fin:
                            actividades.append({
                                "titulo":      a.get("titulo") or a.get("title") or a.get("nombre") or "Sin título",
                                "fecha":       fecha_str,
                    ...
                    pass
            titulo_pdf = f"Agenda · {inicio.strftime('%B %Y').capitalize()}"
            ...
            for a in todos:
                ...
                try:
                    ...
                    pass
            titulo_pdf = f"Agenda  {lunes.strftime('%d/%m')} – {fin.strftime('%d/%m/%Y')}"

        ExportadorPDF.exportar_agenda(actividades, titulo=titulo_pdf,
    ...
    def _render_semana(self):
        ...
        for i, dia, fecha_dia in dias_mostrar:
            ...
            for a in acts_dia:
                ...
                col_map = {"Estudio":C("accent"),"Tarea":C("accent"),"Deporte":C("green"),
                           "Ocio":C("teal"),"Reunión":C("amber"),"Salud":C("red"),"Personal":C("pink")}
                cat_color = next((v for k,v in col_map.items() if k in cat), C("accent"))
                arow = ctk.CTkFrame(col_fr, fg_color=C("card"), corner_radius=8, cursor="hand2")
                arow.pack(fill="x", padx=8, pady=3)
                # Banda de color por categoría
                band_row = ctk.CTkFrame(arow, fg_color="transparent", cursor="hand2")
                band_row.pack(fill="x")
                ctk.CTkFrame(band_row, fg_color=cat_color,
                    width=3, corner_radius=2).pack(side="left", fill="y", padx=(4,0), pady=4)
                info = ctk.CTkFrame(band_row, fg_color="transparent", cursor="hand2")
                info.pack(side="left", fill="x", expand=True, padx=6, pady=4)
                lbl_titulo = ctk.CTkLabel(info, text=a.get("titulo","?")[:20],
                    font=("Helvetica",FS("small"),"bold"), text_color=C("text"),
                    anchor="w", cursor="hand2")
                lbl_titulo.pack(fill="x")
                hora = a.get("hora","")
                desc = a.get("descripcion","") or a.get("description","")
                meta = hora if hora else ""
                if cat:
                    meta = f"{hora}  ·  {cat}" if hora else cat
                if meta:
                    lbl_meta = ctk.CTkLabel(info, text=meta[:28],
                        font=("Helvetica",FS("small")), text_color=cat_color,
                        anchor="w", cursor="hand2")
                    lbl_meta.pack(fill="x")
                if desc:
                    lbl_desc = ctk.CTkLabel(info, text=desc[:32],
                        font=("Helvetica", max(FS("small")-1, 10)), text_color=C("text2"),
                        anchor="w", wraplength=140, justify="left", cursor="hand2")
                    lbl_desc.pack(fill="x")
                # ── Clic en tarjeta de semana → ver info completa ──
                _act_fn = lambda e, _a=a: self._ver_actividad_dia(_a)
                for _w in [arow, band_row, info, lbl_titulo]:
                    _w.bind("<Button-1>", _act_fn)
                # Efecto hover visual
                _orig = C("card")
                def _on_enter(e, f=arow): f.configure(fg_color=C("surface2"))
                def _on_leave(e, f=arow): f.configure(fg_color=C("card"))
                arow.bind("<Enter>", _on_enter)
                arow.bind("<Leave>", _on_leave)
    ...
    def _render_mes(self):
        ...
        for d in range(1, n_dias + 1):
            ...
            cell = ctk.CTkFrame(self.grid_fr,
                ...
                corner_radius=10, width=50, height=64, cursor="hand2")
            cell.grid(row=row_i, column=col_i, padx=2, pady=2, sticky="nsew")
            cell.pack_propagate(False)
            lbl_d = ctk.CTkLabel(cell, text=str(d),
                font=("Helvetica",FS("small"),"bold"),
            ...
            # Puntos de actividad
            if acts:
                dots = ctk.CTkFrame(cell, fg_color="transparent", cursor="hand2")
                dots.pack()
                for a in acts[:3]:
                    cat = a.get("categoria","")
            ...
            _fn = lambda e, _f=fecha_obj: self._click_dia_mes(_f)
            cell.bind("<Button-1>", _fn)
            ...
            for a in acts_dia:
                ...
                if h_ini == h:
                    acard = ctk.CTkFrame(slot,
                        fg_color=C("accent"), corner_radius=8, cursor="hand2")
                    acard.pack(fill="x", padx=(62, 12), pady=3)
                    lbl_a = ctk.CTkLabel(acard,
                        text=f"  {a.get('hora_inicio','?')}–{a.get('hora_fin','?')}  {a.get('titulo','?')}",
                        font=("Helvetica", FS("small"), "bold"),
                        text_color="white", anchor="w", cursor="hand2")
                    lbl_a.pack(anchor="w", padx=8, pady=6)
                    # Clic en tarjeta → ver información
                    _act_fn = lambda e, _a=a: self._ver_actividad_dia(_a)
                    acard.bind("<Button-1>", _act_fn)
                    lbl_a.bind("<Button-1>", _act_fn)
            ctk.CTkFrame(slot, fg_color="transparent", height=4).pack()

    def _click_dia_mes(self, fecha_obj):
        """Cambia a vista Día para la fecha seleccionada en el mes."""
        from datetime import datetime as _dt
        hoy = _dt.now().date()
        self._vista = "dia"
        self._offset = 0
        self._dia_offset = (fecha_obj - hoy).days
        # Actualizar botones de vista
        for v_key, btn in getattr(self, "_vista_btns", {}).items():
            activo = (v_key == "dia")
            btn.configure(
                fg_color=C("accent") if activo else C("surface2"),
                text_color="white" if activo else C("text2"))
        self._refresh()

    def _ver_actividad_dia(self, a):
        """Muestra un diálogo con información completa de la actividad del día."""
        top = ctk.CTkToplevel(self)
        top.title(a.get("titulo", "Actividad"))
        ...
        # Banda de color
        band = ctk.CTkFrame(top, fg_color=color, height=6, corner_radius=0)
        band.pack(fill="x")

        ctk.CTkLabel(top, text=a.get("titulo","Sin título"),
            font=("Helvetica",FS("h2"),"bold"), text_color=C("text"),
            wraplength=440).pack(padx=24, pady=(18,4), anchor="w")
        ...
        desc = a.get("desc") or "Sin descripción."
        ctk.CTkLabel(top, text="📝  Descripción:",
            font=("Helvetica",FS("body"),"bold"), text_color=C("text")).pack(padx=24, anchor="w")
        txt = ctk.CTkTextbox(top, height=100, corner_radius=10,
            font=("Helvetica",FS("body")),
            fg_color=C("surface"), text_color=C("text"),
            border_color=C("border"), border_width=1)
        txt.pack(fill="x", padx=24, pady=(6,12))
        txt.insert("0.0", desc)
        txt.configure(state="disabled")

        lbl_ia = ctk.CTkLabel(top, text="",
            font=("Helvetica",FS("small")), text_color=C("text2"), wraplength=440)
        lbl_ia.pack(padx=24, anchor="w")

        def _pedir_ia():
            lbl_ia.configure(text="⏳  Generando respuesta IA...", text_color=C("accent"))
            top.update()
            try:
                from academic_engine import AcademicEngine
                engine = AcademicEngine({"agenda":[], "materias":{}})
                ...
                txt.delete("0.0","end")
                txt.insert("0.0", resultado)
                txt.configure(state="disabled")
                lbl_ia.configure(text="✅  IA respondió — revisa la descripción.", text_color=C("green"))
            except Exception as ex:
                lbl_ia.configure(text=f"⚠  {ex}", text_color=C("red"))

        br = ctk.CTkFrame(top, fg_color="transparent")
        br.pack(pady=8)
        Btn(br, text="🤖  Pedir ayuda IA", width=180, command=_pedir_ia).pack(side="left", padx=6)
        BtnOutline(br, text="Cerrar", width=100, command=top.destroy).pack(side="left", padx=6)
...
class PanelPomodoro(ctk.CTkFrame):
    def __init__(self, parent, file_key, datos, autostart=False, **kw):
        super().__init__(parent, fg_color=C("bg"), **kw)
        self.file_key = file_key
        self.datos    = datos
        self._seg     = 25*60
