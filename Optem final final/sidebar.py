...
class Sidebar(ctk.CTkFrame):
    ...
    def _pick_avatar(self):
        """Muestra un menú emergente con opciones de perfil."""
        self._show_profile_menu()

    def _show_profile_menu(self):
        """Popup flotante con Cambiar foto y Cerrar sesión."""
        if hasattr(self, "_profile_popup") and self._profile_popup:
            try:
                if self._profile_popup.winfo_exists():
        ...
        btn = self._avatar_btn
        x = btn.winfo_rootx() + btn.winfo_width() + 10
        y = btn.winfo_rooty()
        ...
        frame = ctk.CTkFrame(popup, fg_color=C("surface"),
                             corner_radius=14, border_width=1,
                             border_color=C("border"))
        frame.pack(padx=2, pady=2)

        def _do_cambiar_foto():
            popup.destroy()
            self._profile_popup = None
        ...
        def _do_cerrar_sesion():
            popup.destroy()
            self._profile_popup = None
            from tkinter import messagebox as _mb
            if _mb.askyesno("Cerrar sesión", "¿Deseas cerrar tu sesión?", icon="question"):
                from session_manager import cerrar_sesion
                cerrar_sesion()
                app = self.winfo_toplevel()
                if hasattr(app, "volver_a_login"):
                    app.volver_a_login()

        _items = [
            ("📷  Cambiar foto de perfil", _do_cambiar_foto, C("text")),
            ("🚪  Cerrar sesión",          _do_cerrar_sesion, C("red")),
        ]
        for i, (txt, cmd, tcol) in enumerate(_items):
            b = ctk.CTkButton(frame, text=txt, anchor="w",
                              ...
                              font=("Helvetica", 12),
                              command=cmd)
            b.pack(padx=8, pady=(8 if i == 0 else 2,
                                  8 if i == len(_items) - 1 else 2))
            if i < len(_items) - 1:
                ctk.CTkFrame(frame, fg_color=C("border"), height=1
        ...
        popup.bind_all("<Button-1>", _on_click_outside, add="+")
    ...
    def actualizar_reinscripcion(self, activa: bool):
        """Actualiza el indicador de reinscripción del sidebar en tiempo real."""
        if not hasattr(self, "_reinsc_pill"):
            return
        color = C("green") if activa else C("red")
        texto = "🟢 Reinscripción ABIERTA" if activa else "🔴 Reinscripción CERRADA"
        self._reinsc_pill.configure(fg_color=color)
        self._reinsc_lbl.configure(text=texto)
